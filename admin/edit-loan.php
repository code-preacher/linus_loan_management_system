...
if(isset($_POST['submit'])){
	...
	$total=$amount+$profit;
	$ck=mysqli_query($con,"select id from loan where id='$id'");	
$kc=mysqli_fetch_assoc($ck);
$id=$kc['id'];

$q=$con->prepare("update loan set borrower=?,amount=?,profit=?,total=?,duration_from=?,duration_to=? where id='$id'");
$q->bind_param('siiiss',$bname,$amount,$profit,$total,$durationf,$durationt);
$q->execute();


if($q){
$_SESSION['msg2']="Loan Updated Successfully !!!";
}
else
{
$_SESSION['msg1']="Loan Updating Failed !!!";
}
}
?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<title>Admin  | Edit Loan Issueing</title>
		<meta charset="utf-8" />
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<section id="page-title">
							<div class="row">
								<div class="col-sm-8">
									<h1 class="mainTitle">Admin | Edit Issue Loan</h1>
																	</div>
								<ol class="breadcrumb">
									<li>
										<span>Admin</span>
										...
										<div class="col-lg-8 col-md-12">
											<div class="panel panel-white">
												<div class="panel-heading">
													<h5 class="panel-title">Edit Issue Loan</h5>
												</div>
...
?>
					...
					<input type="text" name="bname" class="form-control"  placeholder="Enter Borrower Name" value="<?php echo htmlentities($data['borrower']);?>">
														</div>
														<div class="form-group">
															<label for="exampleInputPassword1">
															Amount : (Initial Amount:<?php echo "₦".htmlentities($data['amount']);?>)
															</label>
					<input type="number" name="amount" class="form-control"  placeholder="Enter Amount" required="required">
														</div>


														<div class="form-group">
															<label for="exampleInputPassword1">
															Profit : (Initial Profit:<?php echo "₦".htmlentities($data['profit']);?>)
															</label>
					<input type="number" name="profit" class="form-control"  placeholder="Enter Profit" required="required">
														</div>
...
<div class="form-group">
															<label for="exampleInputPassword1">
															Duration From :(Initial Duration:<?php echo htmlentities($data['duration_from']);?>)
															</label>
					<input type="date" name="durationf" class="form-control"  placeholder="Enter Duration" >
														</div>
				
				<div class="form-group">
															<label for="exampleInputPassword1">
														Duration To :(Initial Duration:<?php echo htmlentities($data['duration_to']);?>)
															</label>
					<input type="date" name="durationt" class="form-control"  placeholder="Enter Duration" >
														</div>
