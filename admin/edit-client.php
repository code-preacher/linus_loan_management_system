...
if(isset($_POST['submit'])){
	...
	$ck=mysqli_query($con,"select id from client where id='$id'");	
$kc=mysqli_fetch_assoc($ck);
$id=$kc['id'];

$q=$con->prepare("update client set name = ? ,sex = ? ,address = ? ,occupation = ? , email = ? , number = ? where id='$id' ");
$q->bind_param('ssssss',$name,$sex,$address,$occupation,$email,$number);
$q->execute();


if($q){
$_SESSION['msg2']="Client Updated Successfully !!!";
}
else
{
$_SESSION['msg1']="Client failed Updating !!!";
}
}
?>
<!DOCTYPE html>
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<section id="page-title">
							<div class="row">
								<div class="col-sm-8">
									<h1 class="mainTitle">Admin | Edit Client</h1>
																	</div>
								<ol class="breadcrumb">
									<li>
										<span>Admin</span>
...
?>
					...
					<input type="text" name="name" class="form-control"  placeholder="Enter Client Name" value="<?php echo htmlentities($data['name']);?>">
														</div>
					...
					<input type="text" name="address" class="form-control"  placeholder="Enter Address" value="<?php echo htmlentities($data['address']);?>">
														</div>
	...
	<div class="form-group">
					...
					<input type="text" name="occupation" class="form-control"  placeholder="Enter Occupation" value="<?php echo htmlentities($data['occupation']);?>">
														</div>
					...
					<input type="email" name="email" class="form-control email"  placeholder="Enter Email" value="<?php echo htmlentities($data['email']);?>">
														</div>
															<div class="form-group">
															<label for="exampleInputPassword1">
																Phone Number
															</label>
					<input type="text" name="number" class="form-control"  placeholder="Enter Phone Number" value="<?php echo htmlentities($data['number']);?>">
														</div>
